...
# ===============================
selected_version = render_model_selector(sidebar=True, key="app_model_select")
...
with col_input:
    st.markdown("""
        <div class="custom-card">
            <h3 style="margin-top:0; margin-bottom:12px; font-size:1.2rem; color:#f8fafc;">
                📝 Masukkan Komentar
            </h3>
            <p style="color:#94a3b8; font-size:0.9rem; margin-bottom:16px;">
                Tuliskan kalimat atau komentar bahasa Indonesia untuk dianalisis emosinya secara real-time.
            </p>
        </div>
...
with col_result:
    if btn_predict:
        if not text.strip():
            ...
            st.markdown(f"""
                <div class="custom-card" style="border-left: 4px solid {EMOTION_COLORS.get(top_label, {}).get('hex', '#6366f1')};">
                    <div style="font-size:0.85rem; color:#94a3b8; text-transform:uppercase; font-weight:700; letter-spacing:0.5px;">
                    ...
                    <div style="font-size:0.9rem; color:#cbd5e1;">
                        Model <b>{model_name}</b> yakin sebesar <b>{top_conf*100:.2f}%</b> bahwa komentar ini beremosi <b>{top_label}</b>.
                    </div>
                </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown("""
                <div style="margin-top: 16px;">
                    <h4 style="font-size:1rem; color:#e2e8f0; margin-bottom:12px;">📊 Rincian Probabilitas 6 Emosi:</h4>
                </div>
            """, unsafe_allow_html=True)
        ...
        st.markdown("""
            <div class="custom-card" style="text-align: center; padding: 40px 20px;">
                ...
                <h4 style="color: #cbd5e1; margin-bottom: 6px;">Siap Melakukan Analisis</h4>
                <p style="color: #64748b; font-size: 0.9rem;">
                    Ketik komentar di sebelah kiri lalu klik tombol <b>Analisis Emosi</b> untuk melihat prediksi model.
                </p>
            </div>
        """, unsafe_allow_html=True)