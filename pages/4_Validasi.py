...
render_header(
    ...
    subtitle="Interface Human-in-the-loop untuk verifikasi atau koreksi hasil prediksi model.",
    icon="📝"
)
...
m1, m2 = st.columns(2)
with m1:
    st.markdown(f'<div class="metric-card"><div class="metric-label">⏳ Komentar Belum Divalidasi</div><div class="metric-value" style="color: #f59e0b;">{len(df_belum):,}</div></div>', unsafe_allow_html=True)
with m2:
    st.markdown(f'<div class="metric-card"><div class="metric-label">✅ Komentar Sudah Divalidasi</div><div class="metric-value" style="color: #10b981;">{len(df) - len(df_belum):,}</div></div>', unsafe_allow_html=True)
...
for _, row in df_belum.iterrows():
    ...
    with st.container(border=True):
        ...
        st.markdown(f"🤖 **Prediksi Model:** {badge_html}", unsafe_allow_html=True)
